...
void Tcp_server::test() {
    const unsigned MAX_BATCH = 32;
    struct io_uring_cqe* cqe_arr[MAX_BATCH];
    while(1){
        unsigned ready = io_uring_peek_batch_cqe(&ring, cqe_arr, MAX_BATCH);
        for(int i = 0;i < ready;i++) {
            uint64_t op = ((cqe_data *) cqe_arr[i]->user_data)->op;
            if (op == IORING_OP_SEND)
                handle_send_event(cqe_arr[i]);
            else if (op == IORING_OP_RECV) {
                string message;
                message.clear();
                int fd = handle_receice_event(cqe_arr[i], message);
                //if(!message.empty()){
                //cout << message;
                ...
                handle_listen_event(cqe_arr[i]);
        }
        if(ready) io_uring_cq_advance(&ring,ready);
    }
}
